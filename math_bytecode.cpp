...
parsegen::language build_language() {
  parsegen::language l;
  std::string const space_regex = parsegen::regex::whitespace();
  std::string const maybe_space_regex = space_regex + "?";
  std::string const comment_regex = parsegen::regex::C_style_comment();
  std::string const filler_regex =
    maybe_space_regex + "(" + comment_regex + maybe_space_regex + ")*";
  std::string const int_regex = parsegen::regex::unsigned_integer();
  l.tokens.resize(token_count);
  l.tokens[token_integer] = {
    "integer", int_regex + filler_regex };
  l.tokens[token_floating_point] = {
    "floating_point", int_regex + "(\\.[0-9]*)?([eE][\\-\\+]?[0-9]+)?" + filler_regex };
  l.tokens[token_plus] = {"plus", "\\+" + filler_regex};
  l.tokens[token_minus] = {"minus", "\\-" + filler_regex};
  l.tokens[token_times] = {"times", "\\*" + filler_regex};
  l.tokens[token_divide] = {"divide", "/" + filler_regex};
  l.tokens[token_raise] = {"raise", "\\^" + filler_regex};
  l.tokens[token_assign] = {"assign", "=" + filler_regex};
  l.tokens[token_open_parens] = {"open_parens", "\\(" + filler_regex};
  l.tokens[token_close_parens] = {"close_parens", "\\)" + filler_regex};
  l.tokens[token_open_array] = {"open_array", "\\[" + filler_regex};
  l.tokens[token_close_array] = {"close_array", "\\]" + filler_regex};
  l.tokens[token_double] = {"double", "double" + filler_regex};
  l.tokens[token_const] = {"const", "const" + filler_regex};
  l.tokens[token_reference] = {"reference", "&" + filler_regex};
  l.tokens[token_void] = {"void", "void" + filler_regex};
  l.tokens[token_if] = {"if", "if" + filler_regex};
  l.tokens[token_else] = {"else", "else" + filler_regex};
  l.tokens[token_identifier] = {"identifier", "[_A-Za-z][_A-Za-z0-9]*" + filler_regex};
  l.tokens[token_statement_end] = {"statement_end", ";" + filler_regex};
  l.tokens[token_argument_separator] = {"argument_separator", "," + filler_regex};
  l.tokens[token_open_block] = {"open_block", "{" + filler_regex};
  l.tokens[token_close_block] = {"close_block", "}" + filler_regex};
  l.tokens[token_logical_or] = {"logical_or", "\\|\\|" + filler_regex};
  l.tokens[token_logical_and] = {"logical_and", "&&" + filler_regex};
  l.tokens[token_logical_not] = {"logical_not", "!" + filler_regex};
  l.tokens[token_equal] = {"equal", "==" + filler_regex};
  l.tokens[token_not_equal] = {"not_equal", "!=" + filler_regex};
  l.tokens[token_less] = {"less", "<" + filler_regex};
  l.tokens[token_less_or_equal] = {"less_or_equal", "<=" + filler_regex};
  l.tokens[token_greater] = {"greater", ">" + filler_regex};
  l.tokens[token_greater_or_equal] = {"greater_or_equal", ">=" + filler_regex};
